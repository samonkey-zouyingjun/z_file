...
public class Type2Message extends NtlmMessage {
        ...
        try {
            ...
            if ((flags & 4) != 0) {
                if (targetName == null || targetName.length() == 0) {
                    ...
                    target = (flags & 1) != 0 ? targetName.getBytes("UTF-16LE") : targetName.toUpperCase().getBytes(NtlmMessage.getOEMEncoding());
                }
            }
    ...
    public static int getDefaultFlags(Type1Message type1) {
        ...
        return flags | InputDeviceCompat.SOURCE_TRACKBALL;
    }
    ...
    private void parse(byte[] material) throws IOException {
        ...
        if (bytes.length != 0) {
            target = new String(bytes, (flags & 1) != 0 ? "UTF-16LE" : NtlmMessage.getOEMEncoding());
        }
        setTarget(target);
        for (i = 24; i < 32; i++) {
